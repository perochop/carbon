...
var InputValidation = function InputValidation(ComposedComponent) {
  ...
  function (_ComposedComponent) {
    ...
    function Component() {
      ...
      _defineProperty(_assertThisInitialized(_this), "initValidationInput", function (context) {
        if (!_this.newContextIntegration && context) {
          _this.newContextIntegration = context;

          if (_this._validations() || _this.props.warnings || _this.props.info) {
            // attach the input to the form so the form can track what it needs to validate on submit
            context.addInput(_this._guid, _this.validateFromForm);
          }
        }
      });
        ...
        if (!valid) {
          ...
          if (!_this.state.warning) {
            ...
            if (_this.isAttachedToForm) {
              ...
              _this.newContextIntegration.adjustCount('warning', true);
            } // if input has a tab
      ...
      _defineProperty(_assertThisInitialized(_this), "validateFromForm", function () {
      ...
      _defineProperty(_assertThisInitialized(_this), "updateValidation", function (valid, value, validation) {
        ...
        if (!valid) {
          ...
          if (_this.state.valid) {
            ...
            if (_this.isAttachedToForm) {
              // increment the error count on the form
              _this.newContextIntegration.adjustCount('error', true);
            } // if input has a tab
      ...
      _defineProperty(_assertThisInitialized(_this), "_handleContentChange", function () {
        ...
        if (!_this.state.valid || _this.state.warning || _this.state.info) {
          ...
          if (_this.isAttachedToForm) {
            if (!_this.state.valid) {
              _this.newContextIntegration.adjustCount('error', false);
            }

            if (_this.state.warning) {
              _this.newContextIntegration.adjustCount('warning', false);
            }
      ...
      _this._window = _browser.default.getWindow(); // use this property to store the new context

      _this.newContextIntegration = null; // use the super components state, or create an empty object
    ...
    _createClass(Component, [{
      ...
      get: function get() {
        ...
        var validationContext = _react.default.createElement(_validations.ValidationsContext.Consumer, {
        ...
        return [validationContext, _react.default.createElement(_icon.default, {
          key: "0",
